...
def _plot(
  ...
  ax0.plot(rms, color='white', linewidth=1)
  ...
  if result.activations is not None:
    ...
    # print("colors = ",colors)
    label_acts = result.activations['label']
    for i in range(label_acts.shape[0]):
      label = HARMONIX_LABELS[i]
      # print("label = ",label)
      color = colors[label]
      if not isinstance(color, str):
        color = (color - min_color) / (max_color - min_color)
        color = cmap(color)
      # print("color = ",color)
      ax2.plot(label_acts[i], linewidth=1, color=color)
    ax2.set_xlim(0, len(label_acts[0]) - 1)
    ax2.set_ylim(0, None)
  ...
  if out_dir is not None:
    ...
    fig.savefig(out_dir / f'{result.path.stem}.{ext}', bbox_inches='tight')
